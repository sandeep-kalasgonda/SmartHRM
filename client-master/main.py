...
class Contact(BaseModel):
    co_id: Optional[str] = None  # Added co_id
    co_name: str
...
class ClientWithContacts(BaseModel):
    cl_id: Optional[str] = None  # Added cl_id
    cl_name: str
...
def create_client_with_contacts(client: ClientWithContacts):
    cl_uuid = uuid.uuid4()  # Create a UUID for the client
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        # Insert client data with NULL handling for optional fields
        cursor.execute(
        ...
        for contact in client.contacts:
            co_uuid = uuid.uuid4()  # Create a UUID for each contact
            cursor.execute(
...
def get_clients():
    ...
    try:
        ...
        for client in clients:
            ...
            # Include the co_id for each contact
            for contact in contacts:
                contact['co_id'] = contact['co_id']
            client['contacts'] = contacts
            client['cl_id'] = client['cl_id']  # Include cl_id in the client response
...
def get_client(client_id: str):
    ...
    try:
        ...
        for contact in contacts:
            contact['co_id'] = contact['co_id']
        client['contacts'] = contacts
        client['cl_id'] = client['cl_id']  # Include cl_id in the response
        return client
    except mysql.connector.Error as err:
        logger.error(f"Error: {err}")
