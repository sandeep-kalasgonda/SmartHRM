...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, CommonModule, RouterLink, RouterOutlet,InputSwitchModule,SkeletonModule,ListboxModule,DropdownModule],
...
export class ResumelistComponent implements OnInit {
  ...
  loadClients(): void {
    this.http.get<Client[]>('http://localhost:8888/api/viewer/clients').subscribe(
      (data: Client[]) => {
  ...
  loadPositions(clientId: string): void {
    this.http.get<Position[]>(`http://localhost:8888/api/viewer/positions?client_id=${clientId}`).subscribe(
      (data: Position[]) => {
  ...
  fetchResumes(): void {
    if (this.selectedClientId && this.selectedPosition) {
      ...
      this.http.get<Resume[]>(`http://localhost:8888/api/viewer/resumes?jd_id=${this.selectedPosition}`).subscribe(
        (data: Resume[]) => {
  ...
  updateStatus(resumeId: string, statusType: 'eligible' | 'notEligible' | 'onboarded', value: boolean): void {
    const apiUrl = `http://localhost:8888/resumes/status/${resumeId}?status=${statusType === 'eligible' ? (value ? 'Eligible' : 'Not Eligible') :
