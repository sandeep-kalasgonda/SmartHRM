...
export class CandidateMasterComponent implements OnInit {
    ...
    if (rq_id) {
      this.candidateService.createCandidate(rq_id, candidate).subscribe({
        next: (response) => {
          ...
          this.loadCandidates();
          this.snackBar.open('Candidate added successfully!', 'Close', { duration: 3000 }); // Show success message
        },
        error: (err) => {
          console.error('Error creating candidate:', err);
          this.snackBar.open('Error adding candidate. Please try again.', 'Close', { duration: 3000 }); // Show error message
        }
      });
    } else {
          ...
          if (result) {
            this.candidateService.updateCandidate(candidate.cd_id, result).subscribe({
              next: (response) => {
                ...
                this.loadCandidates();
                this.snackBar.open('Candidate updated successfully!', 'Close', { duration: 3000 }); // Show success message
              },
              error: (err) => {
                console.error('Error updating candidate:', err);
                this.snackBar.open('Error updating candidate. Please try again.', 'Close', { duration: 3000 }); // Show error message
              }
            });
          }
        });
  ...
  deleteCandidate(candidate: Candidate): void {
    if (confirm(`Are you sure you want to delete candidate ${candidate.cd_first_name}?`)) {
      this.candidateService.deleteCandidate(candidate.cd_id).subscribe({
        next: (response) => {
          console.log('Deleted candidate:', response);
          this.loadCandidates();
          this.snackBar.open('Candidate deleted successfully!', 'Close', { duration: 3000 }); // Show success message
        },
        error: (err) => {
          console.error('Error deleting candidate:', err);
          this.snackBar.open('Error deleting candidate. Please try again.', 'Close', { duration: 3000 }); // Show error message
        }
      });
    }
  }
  ...
  filterCandidates(): Candidate[] {
    return this.candidates.filter(candidate =>
      ...
      candidate.cd_email?.toLowerCase().includes(this.searchQuery.toLowerCase()) || // Include email in search
      candidate.cd_phno?.toLowerCase().includes(this.searchQuery.toLowerCase()) // Include phone number in search
    );
  }  
}
